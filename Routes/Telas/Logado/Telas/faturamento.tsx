...
export default function Faturamento() {
    ...
    async function inicio() {
        const empresas = await parceiroSearch('carregarlojaparceira', usuario.loja_parceira);
        console.log('busca de empresas=>', empresas)
        if (empresas.code === 0) {
            setLiberado(true);
        } else {
            setLiberado(false);
    ...
    async function fechamento(id_empresa: string) {
        ...
        console.log('busca de faturamento=>', fat);
        if (fat.code === 0) {
            setLiberado(true)
            setShow(false);
