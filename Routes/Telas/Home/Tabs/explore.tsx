...
export default function Explorer({ }: any) {
  ...
  const { listParceiros, searchByTitle, pesquiza, buscarEmpresas,Base64 } = useContext<any>(AuthLogin);
  ...
  console.log('caminho do arquivo->',Config.configuracoes.pastaProcessos)

  async function searchStore(){
    console.log('Codificação=>','buscarEmpresas')
    const empresas = await buscarEmpresas('buscarEmpresas', 'all');// && console.log(listParceiros)
    console.log('retornoEmpresas=>',empresas);
  }
  try {
    return (
      <ParallaxScrollView
          ...
          <>
            {
              ...
              <View>
                <Text>Erro ao buscar as empresas</Text>
              </View>
            }
          </>
          ...
          <>
            {
                  ...
                  <Collapsible key={parc.id} title={parc.title}>
                    <TouchableOpacity onPress={() => { navigation.navigate(parc.onPress, { loja: parc.title, clienteid: parc.id }) }} style={[Styles.em_linhaHorizontal, Styles.btn, { paddingHorizontal: 0, paddingLeft: -1, paddingVertical: 0, width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]}>
                      <View style={[Styles.em_linhaHorizontal, { width: '33.5%', marginHorizontal: 0 }]}>
                        <Image source={{ uri: Config.configuracoes.pathPadrao + '/src/' + parc.imagem }} style={[Styles.mr_5, { paddingHorizontal: 2.5, backgroundColor: 'green', width: 100, height: 55, borderRadius: 4, borderWidth: 1, borderColor: '#999', padding: 2, resizeMode: 'stretch' }]} />
                      </View>
              ...
              :
              <View style={[Styles.btn,Styles.danger,Styles.em_linhaVertical]}>
                <MaterialCommunityIcons name='bug' size={75} style={[Styles.lbldanger]}/>
                <Text style={[Styles.ft_extraBold,Styles.lbldanger]}>Erro ao buscar empresas!</Text>
                <View style={[Styles.separator,{backgroundColor:'#FFFFFF'}]}/>
                <TouchableOpacity style={[Styles.em_linhaHorizontal,Styles.btn,Styles.secondary,Styles.w95]}
                  onPress={()=>{
                    searchStore();
                  }}
                >
                  <Text style={[Styles.ft_extraBold,Styles.lblsecondary]}>Tentar novamente</Text>
                  <MaterialCommunityIcons name='refresh' size={25} style={[Styles.lblsecondary]}/>
                </TouchableOpacity>
              </View>
            }
          </>
