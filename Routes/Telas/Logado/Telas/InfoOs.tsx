...
import { ScrollView, View, Text,Dimensions, TouchableOpacity, ActivityIndicator, Alert, ToastAndroid} from 'react-native';
...
export default function InfoOs({route,navigation}:any) {
    ...
    const {apresentaModal,osInicada,buscarConfig,buscarOs,nf,dataInicial,dataFinal,nomeCliente,ordemServico,configApp,verificarConexao,fecharModal,isConnectedNetwork,montanteLoja,IniciarOsOffline,getModalStyle,arlterarModal,montantePgmto,buscarCoordenadas,getModalStyleLabel,usuario,IniciarOs,OsIniciada,executarAcao,salvarVariaveis} = useContext<any>(AuthLogin);
    const [totalMontagem,setTotalMontagem] = useState(0);
    const [loadAsync,setLoadAsync] = useState<boolean>(false);
    let cont = 0;

    useEffect(()=>{
        cont ++;
        verificarConfigEmpresa();
        console.log('Execução=>',cont);
    },[]);
    ...
    async function verificarConfigEmpresa(){
        const vrfConn = await verificarConexao();
        //console.log('contando=>',cont);
        if(vrfConn.code !== 0){
            console.log('Sem conexão com a internet!',vrfConn);
            setLoadAsync(true);
        }

        if(isConnectedNetwork === false){
            ToastAndroid.show('Você está no modo offline, Mude para online ou verifique sua conexão, Carregaremos a configuração padrão para esta empresa',ToastAndroid.LONG);
            setLoadAsync(true);
        }else{
            ...
            if(retorno.code === 0){
                ...
                if(iniOs.code === 0){
                    ...
                    if(sv.code === 0){
                        ...
                        if(OsIni.code ===0){
                            console.log('executando aqui=> 81');
                            //setTimeout(() => {
                                navigation.reset({
                                    index:0,
                                    routes:[
            ...
            if(retorno.code === 0){
                ...
                if(iniOs.code === 0){
                    ...
                    if(sv.code === 0){
                        ...
                        console.log('Erro ao salvar variáveis=>',sv);
                    }
                }
            }
        }
    }
    //verifica se há alguma ordem de serviço em andamento online caso não esteja em modo offline
    async function verificarOsEmAndamento(comando:any,nf_:any,dataInicial_:any,dataFinal_:any,nomeCliente_:any,ordemServico_:any,usr:any,status:number){
        const vrfConn = await verificarConexao();
        if(vrfConn.code ===0){
            ...
            console.log(retorno);
            if(retorno.data[0].status ==='OK' && retorno.data[0].statusCode === 200 && retorno.data[0].dados !== undefined &&retorno.data[0].dados !== null){
                apresentaModal(
                    'dialog',
                    'help',
                    'Iniciar trabalho',
                    ()=>(<Text style={[Styles.w100,{textAlign:'center'}]}>{'Existe uma O.S. em andamento online.\n\nDeseja iniciar a O.S. do cliente:'+retorno.data[0].dados[0].title+'?'}</Text>),
                    'default',
                    ()=>{
                        return(
                            <>
    ...
    try {
        if(!loadAsync){
            return(
                <AppLoading msgLoad='Carregando Configurações da empresa...' msgTitle='Carregando.'/>
            );
        }else{
                    ...
                    >
                        ...
                        }
                        {
                            ...
                            <ThemedView style={[Styles.em_linhaVertical,Styles.w100,{alignItems:'center',justifyContent:'flex-start'}]}>
                                <TouchableOpacity style={[Styles.btn,Styles.primary,Styles.em_linhaHorizontal,Styles.w100]}
                                    onPress={()=>{
                                        console.log(route.params.dadosOs.os)
                                        verificarOsEmAndamento('verificarStatusOs',nf,dataInicial !== undefined && dataInicial !== null && dataInicial !== '' ? dataInicial : null,dataFinal !== undefined && dataFinal !== null && dataFinal !== '' ? dataFinal : null,nomeCliente,ordemServico,usuario.id_login[0].id,600)
                                        /*if(item.status){
