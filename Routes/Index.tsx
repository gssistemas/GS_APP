...
export default function Routes() {
  ...
  const [modeOffline, setModeOffline] = useState<boolean>(true);
  const [foregroundNotification, setForegroundNotification] = useState<any>(null);
  const [showNotificationBanner, setShowNotificationBanner] = useState(true);

  async function updateTokenNotification(token_: any, id_user: any) {
  ...
  useEffect(() => {
    ...
    notificationListener.current = Notifications.addNotificationReceivedListener(notification => {
      ...
      console.log('linha 138 =>📩 Notificação recebida:', notification);

      // 👇 Salva a notificação para exibir no banner
  ...
  const registerForPushNotificationsAsync = async () => {
    try {
      ...
      try {
        ...
        usuario !== null && await updateTokenNotification(token.data, usuario.id_login[0].id);
    ...
    if (vc.code === 0) {
      ...
      if (AlApp.code === 0) {
        ...
        if (usuario !== null) {
          ...
          console.log('item validApp=>', validApp);
          if (validApp.code === 0) {
            setDataLoaded(true);
            setStatusLoad(true);
          } else {
  ...
  async function validationApp() {
    ...
    if (vrfConn.mensagem === 'Usando modo Offline') {
      ...
      if (vrfConn.code === 0) {
        const gerarId = await gerarIdUnico();

        if (gerarId.code === 0) {
          //console.warn('Iniciando validação do app=>', gerarId.id);
          if (gerarId.id === null || gerarId.id === undefined || gerarId.id === '') {
            Alert.alert('Erro de validação!', 'ID único do aparelho não encontrado, por favor, refaça o processo de configuração do aplicativo, ou carregue um novo ID único do app. Deseja carregar o id único do app agora?', [
              {
                text: 'Não',
                onPress: () => {
                },
            ...
            setError({ code: 1, mensagem: 'ID único do aparelho não encontrado!', retorno: 'O ID único do aparelho não foi encontrado, por favor, refaça o processo de configuração do aplicativo.' });
            return;
          } else {
            const licenca = await validarApp(gerarId.id);
  ...
  if (!fontsLoaded || !statusLoad || !dataLoaded || !theme) {
    //console.log('Liberado para renderizar?=>', dataLoaded, 'fontes=>', fontsLoaded, 'statusLoad=>', statusLoad, 'theme=>', theme);
      ...
      return (
        <>
          ...
          </Stack.Navigator>
          {
            ...
            <TouchableOpacity style={{ position: 'absolute', top: 0, left: 0, right: 0, backgroundColor: 'red', alignItems: 'center', borderBottomRightRadius: 10, borderBottomLeftRadius: 10 }}
              onPress={() => {
                Alert.alert('Informação!', 'Parece que você está usando o modo "OFFLINE" no app, Isso é recomendado quando não há rede móvel no dispositivo. Caso deseje mudar clique no icone "wifi-off" no canto superior direito em vermelho para desativar o modo offline.');
              }}
            >
              <Text style={[Styles.ft_extraBold, Styles.offlineIndicator, getModalStyleLabel('danger'), { fontSize: 18 }]}>Modo offline ativado!</Text>
            </TouchableOpacity>
          }
          {
            appIsValid === false && userAppMode === false &&
            <View style={[getModalStyle('danger'), { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, flex: 1, alignItems: 'center', justifyContent: 'center' }]}>
              {
                modeOffline === true && userAppMode === false &&
                <TouchableOpacity style={[Styles.em_linhaHorizontal, Styles.btn, Styles.primary, getModalStyle('light'),Styles.w95,{borderRadius:50}]}
                  onPress={() => {
                    setUserAppMode(true);
                  }}
                >
                  <MaterialCommunityIcons name='wifi-off' size={25} style={[getModalStyleLabel('light'), Styles.mr_5, { marginBottom: 0 }]} />
                  <Text style={[getModalStyleLabel('light'), Styles.ft_bold, { fontSize: 24, textAlign: 'center' }]}>{'Ativar modo teste'}</Text>
                </TouchableOpacity>
              }
              ...
              <TouchableOpacity style={[Styles.em_linhaHorizontal, Styles.btn, Styles.success, Styles.w95,{borderRadius:50}]}
                onPress={() => {
          ...
          {showNotificationBanner && foregroundNotification !== null && (
            <TouchableOpacity
              activeOpacity={0.9}
              style={[Styles.btn, Styles.light, {
                position: 'absolute',
                top: (StatusBar.currentHeight || 100) + 10,
                left: 15,
                right: 15,
                borderRadius: 12,
                padding: 15,
              ...
              onPress={() => {
                ...
                // Navegar para tela de notificações ou tela específica
                if (foregroundNotification.data?.screen) {
                  navigation.navigate(foregroundNotification.data.screen);
                } else {
                  navigation.navigate('notifications');
                }
